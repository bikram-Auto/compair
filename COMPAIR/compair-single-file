...
function deleteFolderContentsRecursive(folderPath) {
  const deleted = [];
  try {
...
function getAllFilesRecursive(folderPath, relativePath = "") {
  const files = [];
  try {
    const entries = fs.readdirSync(folderPath);
    for (const entry of entries) {
...
async function compareFolders(folderA, folderB, shouldCopy = true, syncMode = false) {
  const result = {
    folderA,
    folderB,
    uniqueInA: [],
